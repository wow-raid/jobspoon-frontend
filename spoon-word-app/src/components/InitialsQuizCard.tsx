...
const BigJudge = styled.span<{ $kind: 'O' | 'X' }>`
  ...
  ` : css`
    &::before, &::after {
      ...
      top: 50%;
      width: 120%;
      height: calc(var(--qj-stroke) * 1.2);
      background: var(--qj-color);
      border-radius: calc(var(--qj-stroke) * .6);
...
const Tile = styled.div<{ $reveal?: boolean; $ok?: boolean }>`
    ...
    font-family: 'GhanaChocolate','Pretendard','Noto Sans KR',system-ui,sans-serif;
    font-weight: 400;
    font-size: calc(clamp(26px, 3.2vw, 36px) * var(--tile-scale));
...
const TextInput = styled.input`
    ...
    &:focus {
        box-shadow: 0 0 0 3px rgba(62,99,224,.18);
    }
    &::placeholder {
        color: #9db3e8;
    }
...
const StatusBox = styled.span<{ $state?: OX | null }>`
  ...
  transform: translateY(var(--stat-font-nudge));
  line-height: 1;
  color: ${UI.primary};
...
export default function InitialsQuizCard(props: Props) {
    ...
    React.useEffect(() => {
        ...
        const pos  = Math.min(index - 1, QUIZ_LEN - 1);
        const mark: OX = isCorrect ? "O" : "X";

        setStickyProgress(prev => {
            if (prev[pos] === mark) return prev; // 동일하면 스킵
            const next = [...prev];
            next[pos] = mark;
            return next;
        });
    ...
    /* 표시용 진행도(O/X) — 외부 progress 없으면 현재 문항 결과를 반영(정답 공개 시에만) */
    const displayProgress = React.useMemo(() => {
        if (Array.isArray(progress)) {
            return Array.from({ length: QUIZ_LEN }, (_, i) =>
                progress[i] ?? stickyProgress[i] ?? null
            ) as (OX | null)[];
        }
        return stickyProgress;
    }, [progress, stickyProgress]);

    const isLast = index >= QUIZ_LEN;
            ...
            >
                <Card data-showresult={showResult}>
                    ...
                    <Header>
                        <QBox>
                            {(() => {
                                const activeIndex = Math.min(index, QUIZ_LEN); // 1~3
                                 return (
                                       <Dots aria-label="진행 표시">
                                             {Array.from({ length: QUIZ_LEN }).map((_, i) => {
                                               const filled  = i < activeIndex;              // 진행된 것(현재 포함) 채움
                                               return (
                                                   <Dot
                                                       key={i}
                                                       $filled={filled}
                                                       role="img"
                                                       aria-label={`${i + 1}번 ${filled ? "진행됨" : "대기"}`}
                                                   />
                                               );
                                             })}
                                           </Dots>
                            ...
                            <QLabel>{`Q${Math.min(index, QUIZ_LEN)}`}</QLabel>
                    ...
                    <InputDockIn>
                        ...
                        >
                            ...
                            >
                                {showResult ? (isLast ? "결과 보기" : "다음 문제") : "제출하기"}
                            </SubmitBtn>
                        </InputRow>
                    </InputDockIn>
                </Card>
