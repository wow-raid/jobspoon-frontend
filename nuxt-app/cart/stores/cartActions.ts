...
export const cartAction = {
    async requestAddCartToDjango(payload): Promise<AxiosResponse> {
        ...
        try {
            ...
            const response = await djangoAxiosInstance.post('/cart/register', payload);
    ...
    async requestCartListToDjango(email: string):Promise<AxiosResponse> {
        ...
        try {
            ...
            const response = await djangoAxiosInstance.post('/cart/list', {email});
    ...
    async requestDeleteCartItemToDjango(cartItemId: number[]): Promise<void> {
        const {djangoAxiosInstance} = axiosUtility.createAxiosInstances()
        try {
            await djangoAxiosInstance.delete('/cart/delete', { data: cartItemId })
        } catch (error) {
    ...
    async requestCartItemDuplicationCheckToDjango(payload: {email: string, companyReportId: number}): Promise<void> {
        ...
        try{
            ...
            const res = await djangoAxiosInstance.post('/cart/cart-item-duplication-check', { payload })
            return res.data
        }catch(error){
