...
export const managementActions = {
    async requestUserListToDjango() {
        ...
        try {
            const response: AxiosResponse = await djangoAxiosInstance.get('/management/userList'); // Django API 엔드포인트
    ...
    async requestUserLogListToDjango(){
        const {djangoAxiosInstance} = axiosUtility.createAxiosInstances()
        try {
            const response: AxiosResponse = await djangoAxiosInstance.post('/management/userLogList'); // Django API 엔드포인트
    ...
    async grantAdminRoleInDjango(email: string): Promise<any>{
        const {djangoAxiosInstance} = axiosUtility.createAxiosInstances()
        try{
            await djangoAxiosInstance.post('/management/grant-roleType',{'email':email});
        } catch(error){
    ...
    async revokeAdminRoleInDjango(email: string): Promise<any>{
        const {djangoAxiosInstance} = axiosUtility.createAxiosInstances()
        try{
            await djangoAxiosInstance.post('/management/revoke-roleType',{'email':email});
        } catch(error){
            console.error('roleType 변경 실패:',error)
            throw error
        }
