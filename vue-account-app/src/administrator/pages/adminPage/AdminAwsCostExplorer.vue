<!-- src/pages/AwsDailyCostRange.vue -->
<template>
    ...
    <v-row>
      <v-col cols="12" md="10" lg="8">
        <v-card class="pa-4" rounded="xl">
          <div class="text-h6 font-weight-bold mb-2">AWS 비용 조회 (기간 선택)</div>
          <div class="text-body-2 text-medium-emphasis mb-4">
            <strong>YYYY-MM-DD</strong> 또는 숫자만(예: 20251008) 입력해도 자동 교정됩니다. 종료일은 포함 범위입니다.
          </div>
...
import type { AwsDailyCost } from '@/administrator/service/AdminAwsCostExplorer/dto/AdminAwsCostExplorerDto.ts'
import { getAwsDailyCost } from '@/administrator/service/AdminAwsCostExplorer/AdminAwsCostExplorerApi.ts'
...
const monthBuckets = computed<Record<string, AwsDailyCost[]>>(() => {
  const map: Record<string, AwsDailyCost[]> = {}
...
async function fetchDailyCosts() {
  ...
  loading.value = true
  try {
    const list = await getAwsDailyCost(startISO.value, endISO.value)
    rows.value = [...list].sort((a, b) => a.date.localeCompare(b.date))
    drawChart()
  } catch (e) {
...
/* 여러 이벤트가 쌓일 때도 중앙 기준이 되도록 래퍼 정렬 */
